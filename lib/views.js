...
exports.register = function(wa) {
  ...
  app.use(route.get(url('/admin'), admin));
  app.use(route.get(url('/admin/dashboards'), adminDashboards));
  app.use(route.post(url('/admin/dashboards'), adminCreateDashboard));
};
...
function *adminCreateDashboard() {
  ...
  } else if (!params.patterns.length) {
